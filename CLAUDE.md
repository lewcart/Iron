...
- **"Log the workout I just did"** → `log_workout({ name, date?, duration_min?, notes?, exercises: [{ exercise_id | exercise_name, sets: [{ weight, reps?, duration_seconds?, rpe?, band? }] }] })`. Writes a **PERFORMED, COMPLETED session** into history (NOT a routine template — use `create_routine` for templates). The session shows in `get_recent_workouts` and feeds `get_sets_per_muscle` / `get_weekly_summary` automatically (sets are inserted `is_completed=true` with `end_time` stamped, the two conditions the weekly rollup requires).
  - **Auto-create**: an `exercise_name` that doesn't match the catalog auto-creates a custom (`is_custom=true`, manual) exercise; obvious names get inferred primary muscles (e.g. "Banded Glute Bridge"→glutes, "Band Lateral Raise"→delts), ambiguous ones are left empty rather than guessed wrong. Created names come back in `auto_created_exercises[]`. An ambiguous fuzzy match (multiple hits) is NOT auto-created — it returns `MULTIPLE_MATCHES` so you re-call with an `exercise_id`.
  - **weight:0 is valid** (bodyweight / banded): counts toward set + per-muscle tallies but adds 0 to load volume, and never creates a PB (Epley × 0 = 0, so `excluded_from_pb` is left false — no special-casing needed).
  - **Time-mode** exercises log `duration_seconds` instead of `reps`.
  - **Band** info (`band: 'medium'`) is stored in the set `comment` as `"band: medium"` — NOT a new column (the watch-echo contract forbids adding `workout_sets` columns).
  - **Idempotency**: a same-name + same-day session is still created but the response carries a `warning` (it does not block) so you can flag a possible duplicate to Lou.

...
interface Props {
  /** Called after a successful save. Implementations may navigate, close a sheet, etc. */
  onSaved?: () => void;
  /** Variant tweaks the layout (sheet variant trims the outer padding/width). */
  variant?: 'page' | 'sheet';
}

export function InbodyScanForm({ onSaved, variant = 'page' }: Props) {
  ...
  // Sheet variant skips outer page padding (the Sheet container handles it).
  const containerClass = variant === 'sheet' ? 'space-y-4 p-4' : 'px-4 pb-20 space-y-4';

...
async function pushProgressPhoto(r: Record<string, unknown>): Promise<void> {
  ...
  await query(
    `INSERT INTO progress_photos (uuid, blob_url, pose, notes, taken_at, crop_offset_y, crop_offset_x, ab_visibility, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW())
     ON CONFLICT (uuid) DO UPDATE SET
       ...
       ab_visibility = EXCLUDED.ab_visibility,
       updated_at = NOW()`,
    [r.uuid, r.blob_url, r.pose, r.notes, r.taken_at, r.crop_offset_y ?? null, r.crop_offset_x ?? null, r.ab_visibility ?? null],
  );
}

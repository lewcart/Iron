...
import { updateProjectionPhoto } from '@/db/queries';

export const runtime = 'nodejs';

const MAX_MASK_BYTES = 5 * 1024 * 1024;

/** POST /api/projection-photos/[uuid]/mask
 *
 * Same shape as /api/progress-photos/[uuid]/mask. Stores a person-segmentation
 * mask PNG in Vercel Blob and writes the URL to projection_photos.mask_url.
 */
export async function POST(request: NextRequest, ctx: { params: Promise<{ uuid: string }> }) {
  ...
  const blob = await put(`projection-photos/masks/${uuid}-${crypto.randomUUID()}.png`, file, {
    access: 'public',
    contentType: 'image/png',
  });

  const updated = await updateProjectionPhoto(uuid, { mask_url: blob.url });

-- Migration 001: Iron core schema (Module 1)
-- Baseline workout/exercise tables extracted from legacy schema.sql.
-- Safe to apply against an existing DB: all statements use IF NOT EXISTS.
...
-- is_pr was added later via ALTER; keep the idempotent guard for existing DBs
-- whose workout_sets table predates the column.
...
-- dedupe_key was added later via ALTER for idempotent Apple Health imports;
-- keep the guard so existing DBs converge.

...
--      "auto-approved" / "logged" state is NOT stored — it's derived in the
...
--      (cal ±10%, pro -10%/+∞, carb ±15%, fat -15%/+20%).
...
--
-- See: src/app/nutrition/today/AddFoodSheet.tsx (Layer 1 consumer),
...
-- Partial index — we only ever query "pending" rows (nobody asks for the
-- already-approved set), so don't waste index space on the rest.
...
--
-- Stored as JSONB so future-Lewis can edit without another schema migration.
-- Default reflects asymmetric bands: protein under-shoot is a miss but
-- over-shoot is fine; calories over is the harder fail; carb/fat tolerances
-- are wider than protein.
...
--
-- One row per distinct food_name (case-insensitive, trim-normalized). Surfaces
-- the most-recently-logged macros for that food plus a times_logged frequency
-- count. Drives the AddFoodSheet ranking: prefix-match wins, then frequency,
-- then recency.

...
class SyncEngine {
  ...
  //   v2 (2026-06-13): foods + week_meal_ingredients (migration 052). A prior
  //                    note here claimed "brand-new table → no bump needed", but
  //                    that's wrong: existing devices had already advanced
  //                    last_seq past these tables' CDC inserts (the old client
  //                    silently dropped change_log entries for tables it didn't
  //                    yet recognise), so without a reset they never re-pull the
  //                    new rows. "Brand-new table" does NOT exempt a bump.
  private static readonly SYNC_RESET_VERSION = 2;

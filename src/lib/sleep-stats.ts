 ...
 * score over a window). All clock-time arithmetic uses Europe/London because
...
const LONDON_HOUR_FORMAT = new Intl.DateTimeFormat('en-GB', {
  timeZone: 'Europe/London',
  hour: '2-digit',
  hour12: false,
});

const LONDON_MINUTE_FORMAT = new Intl.DateTimeFormat('en-GB', {
  timeZone: 'Europe/London',
  minute: '2-digit',
});

/** Minutes since Europe/London midnight for a given Date (0..1439). */
export function londonClockMinutes(d: Date): number {
  const hh = Number(LONDON_HOUR_FORMAT.format(d));
  const mm = Number(LONDON_MINUTE_FORMAT.format(d));
  return hh * 60 + mm;
}

/**
 * Circular statistics over clock times. Each timestamp's London-local
...
export function circularClockStats(timestamps: Date[]): { mean_min: number; stdev_min: number } {
  ...
  const angles = timestamps.map(d => (londonClockMinutes(d) / 1440) * 2 * Math.PI);
 ...
 * in bed AND the waketime falls at or after 04:00 Europe/London. Naps and
...
export function isMainSleepNight(inBedMin: number, endAtMs: number): boolean {
  ...
  const hour = Number(LONDON_HOUR_FORMAT.format(new Date(endAtMs)));
  return hour >= 4;
}
